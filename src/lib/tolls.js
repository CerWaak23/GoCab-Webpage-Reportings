/**
 * Pipeline de peajes: lee los exports de la carpeta de Drive y los agrega.
 *
 * Vivía dentro de /api/tolls, donde corría en cada request. Ahora vive acá para
 * que lo pueda invocar también el cron (/api/cron/tolls), que es quien debería
 * ejecutarlo: el proceso completo tarda ~7 s y crece con cada archivo nuevo, así
 * que no tiene por qué estar en el camino de una visita.
 *
 * Este módulo no cachea nada — solo calcula. El almacenamiento del resultado es
 * responsabilidad de tolls-snapshot.js.
 */

import { google } from 'googleapis';
import * as XLSX from 'xlsx';

export const TOLLS_FOLDER_ID = '1rn3x_-ixgPqwvUzx6Tkqb0Pzjd6LT-xw';
...
/**
 * Descarga y procesa todos los exports de peajes. Sin caché: cada llamada hace
 * el trabajo completo (~7 s). Devuelve el dataset entero, incluidas las
 * transacciones fila por fila.
 */
export async function computeTolls() {
  const auth = getAuth();
  ...
  console.log(`[tolls] Processed ${transactions.length} transactions in ${Date.now() - t0} ms`);
...
/**
 * Parte el dataset en los dos snapshots que se guardan por separado:
 *
 *  - index:  lo que necesita el dashboard de gestión (91 KB). No lleva las
 *            transacciones, que es lo que pesa.
 *  - plates: las transacciones agrupadas por patente, que es lo que va a leer
 *            el portal de conductores. Cada conductor solo necesita su propia
 *            entrada, así nunca se le manda el dataset completo.
 */
export function splitSnapshots(data, generatedAt) {
  const index = {
    generatedAt,
  ...
  for (const txn of data.transactions) {
    if (!byPlate[txn.plate]) byPlate[txn.plate] = [];
    byPlate[txn.plate].push(txn);
  }
